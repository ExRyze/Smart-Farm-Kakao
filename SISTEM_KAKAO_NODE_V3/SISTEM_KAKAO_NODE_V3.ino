...
// Values
String temperature, moisture, conductivity, ph, nitrogen, phosphorus, potassium;
...
void requestSoilSensor() {
  ...
  if (len >= 17 && response[0] == 0x01 && response[1] == 0x03 && response[2] == 0x0E) {
    ...
    // Parsing Values
    temperature  = String(((response[5] << 8) | response[6]) / 10);
    moisture     = String(((response[3] << 8) | response[4]) / 10);
    conductivity = String(((response[7] << 8) | response[8]) / 10);
    ph           = String(((response[9] << 8) | response[10]) / 10);
    nitrogen     = String(((response[11] << 8) | response[12]) / 10);
    phosphorus   = String(((response[13] << 8) | response[14]) / 10);
    potassium    = String(((response[15] << 8) | response[16]) / 10);
...
void uploadData() {
  ...
  String payload = id + "," + temperature + "," + moisture + "," + conductivity + "," + ph + "," + nitrogen + "," + phosphorus + "," + potassium;
